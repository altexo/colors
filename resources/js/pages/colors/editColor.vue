    ...
    <div class="col-md-12 card card-container">
      <div class="col-md-12">
        <form @submit.prevent="update" @keydown="form.onKeydown($event)">
          ...
          <v-button :loading="form.busy" type="primary"> Actualizar </v-button>
        </form>
      </div>
    </div>
  </div>
...
middleware: "admin",
  ...
  mounted() {
    let color_id = this.$route.params.color_id;
    console.log("colorID: ", color_id);
    axios.get("/api/colores/" + color_id).then((res) => {
      let color = res.data.data;
      this.form.id = color.id;
      this.form.name = color.name;
      this.form.color = color.color;
      this.form.pantone = color.pantone;
      this.form.year = color.year;
    });
  },
  ...
  methods: {
    async update() {
        let color_id = this.$route.params.color_id;
        await this.form.put("/api/colores/"+ color_id);
    },
